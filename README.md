...
5. **必填入参（`params.<名>.required: true`）＝“无入边的段不能要求”＋“有入边的段每条都要给值”** —— ★一条口径的**两个方向**，✗ 缺任一方向都会让读者在某条路上**取不到值**：
   · **无入边的段**（✗ 不限于 `entry`）✗ 不能要求必填 —— 它从不被跳转到达 ⇒ 没有任何地方给它值；
     确实需要在它里显示“随状态变的话” ⇒ 把值改成 `default`／`required:false`，或把注入口放到**被到达的段**。
   · **有入边的段**（包括 `entry`，本仓 `mist-forest` 的入口就**有**入链）⇒ **每一条入边都要带值**（`links[].args` 或规则行），✗ 只覆盖一条不够。
   ★为什么写在这里：**同一个缺陷形态的两种半**都在本仓实测撞过 ——
     ① 「**无入边却要求必填**」：`m3-min-new` 的入口段 `门厅` **至今无任何入边**（我扫过：`fruit-demo` 与 `mist-forest` 的 `entry` **有**入链，`m3-min-new` 的没有）
        ⇒ 它若声明 `required:true` 就会**永远取不到值**（✗ 没有任何入边能给它值）；
     ② 「**有入边却漏一条**」：`m3-min-new` 的 `门厅.敲门` 无 `args`，而 `暗间.条子` 是 `required:true`
        ⇒ 拿到铜钥匙点「敲门」也能进暗间，但那一步 `{{条子}}` 取不到值 ✗ —— 见 `books#19` 的修复）。
     ⇒ 判据在引擎侧（`prose-vocabulary` 的 P4）＋ 本仓 **PR 门**每笔都会跑它 ⇒ 这类问题**在 PR 期就红** ✓。
   ★一条**自查教训**（✗ 供后人避坑）：我初稿写的是“`entry` 没有入链” ⇒ 一核就站不住（本仓 `mist-forest`／`fruit-demo` 的 `entry` **都有**入链）
     ⇒ 真正的口径是**“无入边”**（`entry` 只是它的一种情形）—— ★表述要么覆盖全部情形，要么就举实例 ✓。
...
- **期望分四类** ⇒ 红能直接归因到面；
- **`why` 必填** ⇒ 防"为绿而写用例"；
- **结局类用例锚的是逐字文案** ⇒ **改文案与更新对应用例必须同 PR**（否则那条红是「文案未同步」，不是「未归因」）；
  等 `#1317`（结局结构信号）落地后，可把「到达该结局」（结构格）与「正文」（内容格）分成两格 ⇒ 改文案只红内容格。
